...
//  * @param config
 */
export let trigger = (config: FirebaseConfiguration): Trigger | any => {
...
function createExpressApp(config: FirebaseConfiguration) {
   ...
   */
  if (valueExists(config.staticDirectory) && config.staticDirectory) {
    router.use(express.static(config.staticDirectory));
    router.get('*.*', express.static(config.staticDirectory));
  }
...
function checkCacheControlValue(config: FirebaseConfiguration) {
  let cdnCacheExpiry;        // define vars
  let browserCacheExpiry;
  let staleWhileRevalidate;
      cdnCacheExpiry = 0;    // set vars init values to not clash with future types, eg. staleWhileRevalidate
